    ...
    if self.state == PARTICLESYSTEM_RUNNING then 
        if self.current_nb < self.nb_max then 
            ...
            if body.center == nil then 
                table.insert( self.particles, Particles:init(self.img, Polygon:init(body.pos.x, body.pox.y, body.vertex, body.mass), lifetime, force))
            else
                table.insert( self.particles, Particles:init(self.img, Circle:init(Vector2:init(body.center.x, body.center.y), body.radius, body.mass), lifetime, force))
            end
